...
#
# Two commands run in this container: `srcds_linux64`, the game, and
# `srcds_run_64`, the script that restarts it. Both names start with
# `srcds`, so a check for that substring is satisfied by either.
#
...
EXACT='grep -qsE '\''^srcds_linux(64)?$'\'' /proc/[0-9]*/comm'
...
# named srcds_linux64 is not an applet it knows and exits immediately. A real standalone
# binary takes the name it is invoked under, which is what /proc/N/comm reads.
docker run -d --name "$RUN" debian:stable-slim sh -c '
  g=srcds_; g="${g}linux64"
  cp /bin/sleep "/tmp/$g"
  cp /bin/sleep /tmp/srcds_run_64
  "/tmp/$g" 3600 &
  /tmp/srcds_run_64 3600 &
...
docker exec "$RUN" sh -c 'for p in /proc/[0-9]*; do
  [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/srcds_linux64" ] && kill -9 "${p##*/}" 2>/dev/null
...
for _ in $(seq 1 20); do
  n="$(docker exec "$RUN" sh -c 'c=0; for p in /proc/[0-9]*; do
        [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/srcds_linux64" ] && c=$((c+1)); done; echo "$c"' 2>/dev/null)"
...
if docker exec "$RUN" sh -c 'for p in /proc/[0-9]*; do [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/srcds_run_64" ] && exit 0; done; exit 1' 2>/dev/null; then
