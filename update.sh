...
COMPOSE_FILE="tf2-server-docker-compose.yml"
PROJECT="${COMPOSE_PROJECT_NAME:-tf2}"
DRY_RUN=false
ALLOW_MAJOR=false
for arg in "$@"; do
  case "$arg" in
...
if [ "$new_major" != "$cur_major" ] && [ "$ALLOW_MAJOR" != "true" ]; then
  ...
  echo "  https://github.com/heyvaldemar/tf2-server-docker-compose/releases/tag/$latest" >&2
  exit 3
fi
